...
# Gradient Boosting
knn_model = KNeighborsClassifier(n_neighbors=35, weights='distance')#algorithm="auto",n_jobs=-1,n_neighbors=2,weights="distance")

knn_model = knn_model.fit(x_train,y_train)

y_probability_train = knn_model.predict_proba(x_train)
y_probability_test = knn_model.predict_proba(x_test)
...
print 'Best threshold: ', best_threshold # 0.51
print 'Best Accuracy: ', best_accuracy # 0.776536312849

y_probability_test_final = knn_model.predict_proba(data_test_clean)
...
prediction_file = open("RESULTS_knn_tune.csv", "wb")
