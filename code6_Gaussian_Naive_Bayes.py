...
# Gradient Boosting
gnb_model = GaussianNB()#create Naive Bayes classifier - no hyperparameters to tine

gnb_model = gnb_model.fit(x_train,y_train)

y_probability_train = gnb_model.predict_proba(x_train)
y_probability_test = gnb_model.predict_proba(x_test)
...
print 'Best threshold: ', best_threshold # 0.79
print 'Best Accuracy: ', best_accuracy #0.865921787709

y_probability_test_final = gnb_model.predict_proba(data_test_clean)
...
prediction_file = open("RESULTS_gnb.csv", "wb")
